async function loadSettings() {
    try {
        ...
        if (!flaskResponse.ok) throw new Error("خطا در دریافت تنظیمات Flask");
        ...
        if (!wgResponse.ok) throw new Error("خطا در دریافت کانفیگ های وایرگارد");
        ...
        console.error('خطا در اپدیت تنظیمات:', error);
        showAlert(`خطا در اپدیت تنظیمات: ${error.message}`);
    }
}
    ...
    try {
        ...
        if (isNaN(port) || port < 1 || port > 65535) {
            showAlert("پورت باید بین 1 یا 65535 باشد");
            return;
        }
        ...
        showAlert(`خطای اپدیت flask: ${error.message}`);
    }
});
    ...
    document.getElementById('updateWGForm').addEventListener('submit', async (event) => {
        ...
        try {
            ...
            showAlert(`خطای اپدیت کانفیگ وایرگارد: ${error.message}`);
        }
    });
